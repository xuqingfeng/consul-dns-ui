...
export default {
  ...
  methods: {
    getServices() {
      fetch('http://' + consul_address + '/v1/agent/services')
        .then(async response => {
    ...
    putService() {
      if (this.serviceFocused.Port > 0) {
        ...
        fetch('http://' + consul_address + '/v1/agent/service/register', requestOptions)
          .then(async response => {
    ...
    deRegisterService(id) {
      ...
      fetch('http://' + consul_address + '/v1/agent/service/deregister/' + id, requestOptions)
        .then(async response => {
          if (!response.ok) {
